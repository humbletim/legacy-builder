...
# 2. Generate build-info.js and then bundle the React Native code and assets
echo "Generating build-info.js..."
BUILD_DATE=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
GIT_HASH=$(git rev-parse --short HEAD)
cat > src/build-info.js << EOL
// This file is generated by the repeat.bash script. Do not edit.
const buildInfo = {
  date: "${BUILD_DATE}",
  hash: "${GIT_HASH}",
};
...
mkdir -p dist
APK_PATH="workspace/AwesomeProject/android/app/build/outputs/apk/release/app-release.apk"
if [ -f "$APK_PATH" ]; then
    cp "$APK_PATH" "dist/AwesomeProject-release.apk"
else
...
echo "adb install -r dist/AwesomeProject-release.apk"
